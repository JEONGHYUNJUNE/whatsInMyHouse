...
function AddItemSheet({ data, initialSpaceId, profileId, demoMode, onClose, onSaved }: { data: AppData; initialSpaceId: string | null; profileId: string; demoMode: boolean; onClose: () => void; onSaved: () => Promise<void> }) {
  ...
  const [webCandidates, setWebCandidates] = useState<{ name: string; description: string; url: string }[]>([])
  ...
  const findBarcode = async (nextBarcode = barcode) => {
    ...
    setWebCandidates([])
    setBusy(true)
    try { const found = await lookupBarcode(nextBarcode, profileId, data.kitchen.id); if (found && 'name' in found) { setNeedsSharedReview(Boolean(found.needsSharedReview)); setCatalogProductId(found.catalogId); setName(found.name); setCategory(found.category); if (/채소|과일/.test(found.category)) { setDeadlineType('purchase'); setRecommendedDays(recommendedFreshDays(found.name, found.category)) } changeUnit(found.unit); setBarcodeImageUrl(found.imageUrl); setPreview(found.imageUrl) } else if (found?.candidates?.length) { setNeedsSharedReview(true); setCatalogProductId(null); setWebCandidates(found.candidates) } else { setNeedsSharedReview(true); setCatalogProductId(null); void showAppAlert('처음 등록하는 상품이에요. 입력한 상품명은 관리자 검토 후 공용 바코드 정보로 활용될 수 있어요.', '새로운 바코드 상품') } } finally { setBusy(false) }
  }
  useEffect(() => {
  ...
  return <div className="sheet-backdrop" onMouseDown={(e) => e.target === e.currentTarget && onClose()}><section className="add-sheet"><div className="sheet-handle" /><div className="sheet-head"><div><p>10초 안에 빠르게</p><h2>식재료 추가</h2></div><button onClick={onClose}><X /></button></div>
    ...
    {mode === 'barcode' && <><BarcodeCameraScanner onDetected={findBarcode} /><details className="barcode-manual"><summary>번호를 직접 입력할게요</summary><div><input inputMode="numeric" value={barcode} onChange={(e) => setBarcode(e.target.value.replace(/\D/g, ''))} placeholder="880..." /><button onClick={() => void findBarcode()}>조회</button></div></details>{webCandidates.length > 0 && <section className="barcode-web-candidates"><div><Search /><span><b>웹 검색에서 상품 후보를 찾았어요</b><small>최종 검색 결과이므로 실제 상품이 맞는지 확인해 주세요.</small></span></div>{webCandidates.map((candidate) => <article key={candidate.url}><div><b>{candidate.name}</b><p>{candidate.description || '상품 설명이 제공되지 않았어요.'}</p><a href={candidate.url} target="_blank" rel="noreferrer">검색 결과 확인</a></div><button type="button" onClick={() => { setName(candidate.name); setWebCandidates([]) }}><Check /> 이 이름 사용</button></article>)}</section>}</>}
