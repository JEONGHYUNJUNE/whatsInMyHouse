...
function BarcodeAdminSheet({ onClose }: { onClose: () => void }) {
  ...
  const openCreateProduct = () => { closeEditor(); setAdminTab('catalog'); setCreatingProduct(true); setName(''); setBrand(''); setCategory(''); setUnit('개'); setImageUrl('') }
  ...
  const createProduct = async () => {
    ...
    setBusy(true); setError('')
    try { const result = await createSharedBarcodeProduct(newBarcode, { productName: name, brand, category, unit, imageUrl }); closeEditor(); await load(); if (result.imageWarning) setError(`상품은 등록했지만 사진은 저장하지 못했어요: ${result.imageWarning}`) }
